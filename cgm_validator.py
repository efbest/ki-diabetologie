...
# ─────────────────────────────────────────────────────────────
#  V2.0 — NEUE ANALYSE-MODULE
#  Wissenschaftliche Grundlage: NotebookLM-Literaturanalyse
#  (CGM-Signalverarbeitung, Kalman-Filter, Bias-Detektion)
# ─────────────────────────────────────────────────────────────

def analyze_signal_freeze(df, interval_min):
    """
    Modul 6: Signal-Freeze Detektor (Varianz-Check).

    Biologische Signale fluktuieren immer leicht. Wenn der Hardware-Filter
    zu aggressiv glättet, verschwindet das Mikro-Rauschen vollständig —
    das Signal ist dann "eingefroren" auf einem Modell-Mittelwert, nicht
    auf einer echten Messung.

    Methode: Lokale Standardabweichung σ über ein rollierendes Fenster.
    Wenn σ < ε über mehrere aufeinanderfolgende Punkte, ist das Signal
    künstlich geglättet.

    Schwellenwert: σ < 0.15 mg/dL (extrem niedrig — normales biologisches
    Rauschen liegt typischerweise bei 0.5–2.0 mg/dL für FL3-Daten).

    Wichtig: Kurze Plateaus (1–2 Punkte) sind normal (Kalman-Konvergenz bei
    stabiler Glukose). Erst ≥5 aufeinanderfolgende Freeze-Punkte (= 5 min
    bei 1-min-Daten) werden als kritisch gewertet.
    """
    df = df.copy()
    df["freeze_flag"] = False
    df["local_sigma"] = np.nan

    window = max(5, 10 // interval_min)   # 10 min Fenster
    sigma_thresh = 0.15                   # mg/dL — empirisch kalibriert
    min_run = max(5, 5 // interval_min)   # Mindest-Dauer für kritische Meldung
    ...
    for i in range(window // 2, n - window // 2):
        lo = max(0, i - window // 2)
        hi = min(n, i + window // 2)
        sigma = np.std(glucose[lo:hi])
        df.iloc[i, df.columns.get_loc("local_sigma")] = sigma

    df["local_sigma"] = df["local_sigma"].ffill().bfill()

    # Runs mit σ < Schwelle identifizieren
    freeze_candidate = df["local_sigma"] < sigma_thresh

    # Nur Runs >= min_run flaggen
    in_run = False
    run_start = 0
    for i in range(n):
        if freeze_candidate.iloc[i] and not in_run:
            in_run = True
            run_start = i
        elif not freeze_candidate.iloc[i] and in_run:
            run_len = i - run_start
            if run_len >= min_run:
                df.iloc[run_start:i, df.columns.get_loc("freeze_flag")] = True
            in_run = False
    if in_run:
        run_len = n - run_start
        if run_len >= min_run:
            df.iloc[run_start:n, df.columns.get_loc("freeze_flag")] = True

    return df


def analyze_roc_saturation(df, interval_min):
    """
    Modul 7: RoC-Sättigungsprüfung (Velocity Saturation / Dynamic Lag Alert).

    Wenn der interne Hardware-Filter die Änderungsrate künstlich begrenzt
    (Slew Rate Limiting), kleben mehrere aufeinanderfolgende RoC-Werte
    exakt am selben Maximalwert. Eine "perfekte" Gerade während eines
    steilen Anstiegs ist physiologisch extrem unwahrscheinlich.

    Methode:
    1. Berechne RoC = Δglukose pro Intervall
    2. Prüfe ob RoC über ≥3 aufeinanderfolgende Punkte exakt gleich ist
       (Toleranz: ±0.05 mg/dL) UND der Absolutwert > Schwelle
    3. Zusatz: Prüfe ob RoC konstant am theoretischen Physiologie-Maximum bleibt

    Physiologisches Maximum: ~3 mg/dL/min (= 3 bei 1-min, 15 bei 5-min Daten).
    Wenn RoC konstant exakt an diesem Limit ist, bremst der Filter den realen
    Anstieg aus — "Dynamic Lag Alert".
    """
    df = df.copy()
    df["roc_saturation_flag"] = False
    df["roc_value"] = np.nan
    ...
    for i in range(1, n):
        roc[i] = glucose[i] - glucose[i - 1]
    df["roc_value"] = roc

    # Schwellen
    roc_abs_min = 1.5 if interval_min == 1 else 7.5   # Mindest-RoC für Sättigungsprüfung
    roc_phys_max = 3.0 if interval_min == 1 else 15.0  # Physiologisches Maximum
    tolerance = 0.05                                    # mg/dL Toleranz für "gleich"
    min_run = 3                                         # Min. aufeinanderfolgende gleiche RoC

    i = 1
    while i < n - min_run:
        if np.isnan(roc[i]) or abs(roc[i]) < roc_abs_min:
            i += 1
            continue

        # Suche Gleichlauf
        j = i + 1
        while j < n and not np.isnan(roc[j]) and abs(roc[j] - roc[i]) <= tolerance:
            j += 1

        run_len = j - i
        if run_len >= min_run:
            df.iloc[i:j, df.columns.get_loc("roc_saturation_flag")] = True
        i = j if j > i else i + 1

    return df


def analyze_acceleration(df, interval_min):
    """
    Modul 8: Beschleunigungsanalyse (2. Ableitung).

    Natürliche Glukoseverläufe haben weiche Übergänge in der Beschleunigung
    (d²G/dt²). Aggressive digitale Filter erzeugen abrupte "Knicke" in der
    Beschleunigung — die Geschwindigkeit (1. Ableitung) bleibt fast konstant,
    während die Beschleunigung plötzlich springt.

    Methode:
    1. Erste Ableitung: v(t) = G(t) - G(t-1)
    2. Zweite Ableitung: a(t) = v(t) - v(t-1) = G(t) - 2*G(t-1) + G(t-2)
    3. Normalisierung: |a(t)| / (|v(t)| + ε)
       → Hoher Wert = abrupte Richtungsänderung bei konstanter Geschwindigkeit

    Schwellenwert: Normalisierte Beschleunigung > 2.0 bei gleichzeitig
    hoher Geschwindigkeit (|v| > 1.0 mg/dL/min) als Indikator für
    Filter-induzierte Kurvenform-Anomalie.
    """
    df = df.copy()
    df["accel_flag"] = False
    df["acceleration"] = np.nan
    df["accel_norm"] = np.nan
    ...
    for i in range(1, n):
        velocity[i] = glucose[i] - glucose[i - 1]
    for i in range(2, n):
        accel[i] = glucose[i] - 2 * glucose[i - 1] + glucose[i - 2]

    df["acceleration"] = accel

    # Normalisierte Beschleunigung
    eps = 0.5  # Vermeidet Division durch Null bei stabilem Signal
    accel_norm = np.abs(accel) / (np.abs(velocity) + eps)
    df["accel_norm"] = accel_norm

    # Flagging: hohe norm. Beschleunigung + nennenswerte Geschwindigkeit
    vel_thresh = 1.0 if interval_min == 1 else 5.0
    accel_norm_thresh = 2.0

    for i in range(2, n):
        if (not np.isnan(accel_norm[i]) and
                accel_norm[i] > accel_norm_thresh and
                not np.isnan(velocity[i]) and
                abs(velocity[i]) > vel_thresh):
            df.iloc[i, df.columns.get_loc("accel_flag")] = True

    return df


def parse_bolus_from_zip(zip_bytes):
    """
    Extrahiert Mahlzeiten-Zeitstempel aus bolus_data_*.csv einer Glooko-ZIP.
    Gibt DataFrame mit Spalten [datetime, carbs_g] zurück (nur KH > 0).
    """
    from io import StringIO
    try:
    ...
    bolus_names = sorted([
        ...
        if re.search(r'bolus[_\s-]?data[_\s-]?\d*\.(csv|txt)$', n, re.IGNORECASE)
    ])
    if not bolus_names:
        return pd.DataFrame()

    segments = []
    for bname in bolus_names:
        try:
            raw = zf.read(bname)
            ...
            header_idx = 0
            for i, line in enumerate(lines[:10]):
                if "zeitstempel" in line.lower() or "timestamp" in line.lower():
                    header_idx = i
                    break

            df_raw = pd.read_csv(StringIO(content), skiprows=header_idx,
                                 sep=",", on_bad_lines="skip", dtype=str)
            df_raw.columns = [c.strip().lstrip('\ufeff').strip('"').strip()
                               for c in df_raw.columns]

            dt_col = next((c for c in df_raw.columns
                           if "zeitstempel" in c.lower() or "timestamp" in c.lower()), None)
            kh_col = next((c for c in df_raw.columns
                           if "kh" in c.lower() or "kohlenhydrat" in c.lower()
                           or "carb" in c.lower()), None)

            if not dt_col or not kh_col:
                continue

            df_b = df_raw[[dt_col, kh_col]].copy()
            df_b.columns = ["datetime", "carbs_g"]
            df_b["carbs_g"] = df_b["carbs_g"].str.replace(",", ".").str.extract(r"([\d.]+)")[0]
            df_b["carbs_g"] = pd.to_numeric(df_b["carbs_g"], errors="coerce")
            df_b = df_b.dropna(subset=["carbs_g"])
            df_b = df_b[df_b["carbs_g"] > 0]

            dt_parsed = pd.to_datetime(df_b["datetime"],
                                       format="%d.%m.%Y %H:%M", errors="coerce")
            if dt_parsed.isna().all():
                dt_parsed = pd.to_datetime(df_b["datetime"], dayfirst=True,
                                           infer_datetime_format=True, errors="coerce")
            df_b["datetime"] = dt_parsed
            df_b = df_b.dropna(subset=["datetime"])
            segments.append(df_b)
...
def analyze_meal_stress_test(df, bolus_df, interval_min):
    """
    Modul 9: Physiologischer Mahlzeiten-Stress-Test.

    Nach einer Mahlzeit (Bolus mit KH > 0) muss der Glukosewert innerhalb
    von 15–45 Minuten messbar ansteigen. Wenn die RoC im 15-Minuten-Fenster
    nach dem Bolus nahe 0 bleibt, obwohl Kohlenhydrate gegessen wurden,
    deutet das auf Hardware-induzierten Bias hin: die Glättung "verschluckt"
    den prandialen Anstieg.

    Bedingung:
    - Glukose zum Zeitpunkt des Bolus: < 200 mg/dL (kein Hyperglykämie-Plateau)
    - Kohlenhydrate: ≥ 10 g (physiologisch wirksame Mahlzeit)
    - Fenster: 5–20 min nach Bolus (Lag-Zeit überspringen)
    - Erwartete RoC: > 0.5 mg/dL/min (= mindestens 7.5 mg/dL Anstieg in 15 min)
    - Warnung wenn: max. RoC im Fenster < 0.3 mg/dL/min

    Gibt DataFrame der auffälligen Bolus-Ereignisse zurück.
    """
    if bolus_df is None or len(bolus_df) == 0 or len(df) == 0:
        return pd.DataFrame()

    results = []
    lag_min = 5    # Physiologischer Lag — erste 5 min überspringen
    window_min = 20  # Auswertungsfenster: 5–25 min nach Bolus
    min_carbs = 10   # Mindest-KH für Test
    roc_warn = 0.3   # mg/dL/min — Warnschwelle

    for _, bolus in bolus_df.iterrows():
        if bolus["carbs_g"] < min_carbs:
            continue

        t0 = bolus["datetime"]
        t_win_start = t0 + pd.Timedelta(minutes=lag_min)
        t_win_end   = t0 + pd.Timedelta(minutes=lag_min + window_min)

        # Glukose zum Bolus-Zeitpunkt
        t0_data = df[abs((df["datetime"] - t0).dt.total_seconds()) <= 120]
        if len(t0_data) == 0:
            continue
        gluc_at_bolus = t0_data["glucose"].iloc[0]

        if gluc_at_bolus >= 200:  # Bereits hyperglykämisch — Anstieg nicht erwartet
            continue

        # Fenster-Daten
        window_data = df[(df["datetime"] >= t_win_start) & (df["datetime"] <= t_win_end)]
        if len(window_data) < 3:
            continue

        # Maximale RoC im Fenster
        roc_vals = window_data["glucose"].diff().dropna()
        if interval_min > 1:
            roc_vals = roc_vals / interval_min
        max_roc = roc_vals.max()

        if max_roc < roc_warn:
            results.append({
                "Mahlzeit (Zeitstempel)": t0.strftime("%d.%m.%Y %H:%M"),
                "KH (g)": bolus["carbs_g"],
                "Glukose bei Bolus": f"{gluc_at_bolus:.0f} mg/dL",
                "Max. RoC im Fenster": f"{max_roc:.2f} mg/dL/min",
                "Befund": "⚠ Kein prandialer Anstieg erkannt — Hardware-Bias möglich",
            })
...
def compute_quality_score(df, gaps_df, interval_min):
    """Berechnet einen Gesamt-Qualitätsscore (0–100) — v2.0 mit neuen Modulen."""
    n = len(df)
    if n == 0:
        return 0
    ...
    n_freeze = df["freeze_flag"].sum() if "freeze_flag" in df.columns else 0
    n_sat    = df["roc_saturation_flag"].sum() if "roc_saturation_flag" in df.columns else 0
    n_accel  = df["accel_flag"].sum() if "accel_flag" in df.columns else 0

    penalty = (
        min((n_roc   / n) *  800, 40) +
        min( n_gap   *  0.5,      15) +
        min((n_plat  / n) *  400, 10) +
        min((n_stat  / n) * 1500, 10) +
        min((n_comp  / n) * 1000,  8) +
        min((n_freeze/ n) * 2000,  8) +   # Freeze: schwerer gewichtet
        min((n_sat   / n) * 1000,  5) +
        min((n_accel / n) *  500,  4) +
        0
    )
...
def generate_pdf_report(df, meta, score, gap_count, wear_df=None, sensors=None,
                        meal_stress_df=None, bolus_df=None):
    ...
    # ── V2.0 — Signal-Qualität
    elements.append(Paragraph("Signal-Qualität — v2.0 Analyse-Module", style_h2))
    elements.append(Paragraph(
        "Diese Module erkennen Hardware-induzierten Bias durch den internen Kalman-Filter. "
        "Sie warnen — korrigieren nicht. Die Entscheidung liegt beim Arzt.",
        style_muted))
    elements.append(Spacer(1, 0.3*cm))

    n_freeze = int(df["freeze_flag"].sum()) if "freeze_flag" in df.columns else 0
    n_sat    = int(df["roc_saturation_flag"].sum()) if "roc_saturation_flag" in df.columns else 0
    n_accel  = int(df["accel_flag"].sum()) if "accel_flag" in df.columns else 0
    n_meal_warn = len(meal_stress_df) if meal_stress_df is not None and len(meal_stress_df) > 0 else 0
    n_meals_total = len(bolus_df) if bolus_df is not None and len(bolus_df) > 0 else 0
    n = max(meta["n"], 1)

    v2_data = [
        ["Modul", "Befunde", "Anteil", "Bewertung", "Interpretation"],
        ["M6 — Signal-Freeze",
         f"{n_freeze} Punkte",
         f"{100*n_freeze/n:.2f}%",
         "⚠ Prüfen" if n_freeze > 0 else "✓ OK",
         "Lokales σ < 0.15 mg/dL über ≥5 Punkte"],
        ["M7 — RoC-Sättigung",
         f"{n_sat} Punkte",
         f"{100*n_sat/n:.2f}%",
         "⚠ Prüfen" if n_sat > 0 else "✓ OK",
         "Konstante RoC ±0.05 mg/dL über ≥3 Punkte"],
        ["M8 — Beschleunigung",
         f"{n_accel} Punkte",
         f"{100*n_accel/n:.2f}%",
         "⚠ Prüfen" if n_accel > 0 else "✓ OK",
         "Norm. Beschleunigung > 2.0 bei |v| > 1.0"],
        ["M9 — Mahlzeiten-Test",
         f"{n_meal_warn} / {n_meals_total} Mahlzeiten" if n_meals_total > 0 else "Keine Daten",
         f"{100*n_meal_warn/max(n_meals_total,1):.1f}%" if n_meals_total > 0 else "—",
         "⚠ Prüfen" if n_meal_warn > 0 else ("✓ OK" if n_meals_total > 0 else "— n/a"),
         "Kein prandialer Anstieg nach ≥10g KH"],
    ]

    v2_col_widths = [4*cm, 3*cm, 2*cm, 2.5*cm, 4.5*cm]
    v2_tbl = Table(v2_data, colWidths=v2_col_widths)
    v2_style = TableStyle([
        ("BACKGROUND",    (0, 0), (-1, 0),  CLR_PRIMARY),
        ("TEXTCOLOR",     (0, 0), (-1, 0),  colors.white),
        ("FONTNAME",      (0, 0), (-1, 0),  "Helvetica-Bold"),
    ...
    v2_tbl.setStyle(v2_style)
    elements.append(v2_tbl)
    elements.append(Spacer(1, 0.3*cm))

    # Signal-Freeze Plot (wenn vorhanden)
    if "local_sigma" in df.columns and n_freeze > 0:
        elements.append(Paragraph("Signal-Freeze: Lokale Standardabweichung über Zeit", style_muted))
        fig_fr, ax_fr = plt.subplots(figsize=(14, 2.5))
        fig_fr.patch.set_facecolor("#FFFFFF")
        ax_fr.set_facecolor("#FAFBFD")
        ax_fr.plot(df["datetime"], df["local_sigma"].clip(0, 35),
                   color="#0071BC", linewidth=0.6, alpha=0.8)
        ax_fr.axhline(0.15, color="#DC2626", linewidth=1.2,
                      linestyle="--", label="Schwelle 0.15 mg/dL")
        freeze_pts = df[df["freeze_flag"]] if "freeze_flag" in df.columns else pd.DataFrame()
        if len(freeze_pts):
            ax_fr.scatter(freeze_pts["datetime"],
                          freeze_pts["local_sigma"].clip(0, 35),
                          c="#DC2626", s=8, zorder=5, alpha=0.7)
        ax_fr.set_ylabel("Lokales σ (mg/dL)", fontsize=8)
        ax_fr.set_title("M6 — Signal-Freeze Detektor", fontsize=9, fontweight="bold")
        ax_fr.legend(fontsize=7)
        for spine in ax_fr.spines.values():
            spine.set_edgecolor("#DDE3ED")
        ax_fr.tick_params(labelsize=7)
        plt.tight_layout()
        elements.append(fig_to_image(fig_fr, w=16*cm, h=3*cm))
        plt.close(fig_fr)

    # Mahlzeiten-Stress-Test Tabelle (wenn Befunde vorhanden)
    if meal_stress_df is not None and len(meal_stress_df) > 0:
        elements.append(Spacer(1, 0.2*cm))
        elements.append(Paragraph("M9 — Mahlzeiten ohne prandialen Anstieg (Details)", style_muted))
        meal_rows = [["Mahlzeit", "KH (g)", "Glukose bei Bolus", "Max. RoC", "Befund"]]
        for _, row in meal_stress_df.iterrows():
            meal_rows.append([
                str(row.get("Mahlzeit (Zeitstempel)", "")),
                str(row.get("KH (g)", "")),
                str(row.get("Glukose bei Bolus", "")),
                str(row.get("Max. RoC im Fenster", "")),
                "⚠ Kein Anstieg",
            ])
        meal_tbl = Table(meal_rows, colWidths=[3.5*cm, 2*cm, 3.5*cm, 3*cm, 4*cm])
        meal_tbl.setStyle(make_tbl_style())
        elements.append(meal_tbl)
    ...
    elements.append(Paragraph(
        "CGM Validator v2.0 — Lokal erstellt. Keine Daten verlassen diesen Rechner. "
...
if not uploaded_files:
    ...
    st.markdown("**v1 — Klassische Qualitätsprüfung**")
    ...
    st.markdown("**v2 — Signal-Qualität &amp; Hardware-Bias (neu)**")
    col4, col5, col6, col7 = st.columns(4)
    with col4:
        st.markdown('<div class="info-card"><strong>❄️ Signal-Freeze</strong><br>Lokales σ &lt; 0.15 mg/dL — Signal "totgefiltert"</div>', unsafe_allow_html=True)
    with col5:
        st.markdown('<div class="info-card"><strong>⚡ RoC-Sättigung</strong><br>Konstante Änderungsrate — Dynamic Lag Alert</div>', unsafe_allow_html=True)
    with col6:
        st.markdown('<div class="info-card"><strong>📐 Beschleunigung</strong><br>2. Ableitung — abrupte Kurvenform-Knicke</div>', unsafe_allow_html=True)
    with col7:
        st.markdown('<div class="info-card"><strong>🍽️ Mahlzeiten-Test</strong><br>Kein prandialer Anstieg nach Bolus — Bias-Warnung</div>', unsafe_allow_html=True)
    st.stop()
...
# ── V2.0: Neue Signal-Qualitäts-Module
df = analyze_signal_freeze(df, interval_min)
df = analyze_roc_saturation(df, interval_min)
df = analyze_acceleration(df, interval_min)

# ── V2.0: Mahlzeiten-Stress-Test (nur bei Glooko-ZIP mit Bolus-Daten)
bolus_df = pd.DataFrame()
_zip_bytes_for_bolus = None
for uf in uploaded_files:
    if uf.name.lower().endswith(".zip"):
        try:
            uf.seek(0)
            _zip_bytes_for_bolus = uf.read()
            break
        except Exception:
            pass

if _zip_bytes_for_bolus:
    bolus_df = parse_bolus_from_zip(_zip_bytes_for_bolus)

meal_stress_df = analyze_meal_stress_test(df, bolus_df, interval_min)
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
    ...
    "🚨 Fehler-Detail", "🧪 Signal-Qualität", "🩹 Liegezeit", "📄 Report"
])
...
with tab5:
    st.markdown("### 🧪 Signal-Qualität — v2.0 Analyse-Module")
    st.markdown(
        '<div class="info-card">Diese Module erkennen <strong>Hardware-induzierten Bias</strong> '
        'durch den internen Kalman-Filter des Sensors. Sie warnen — korrigieren nicht. '
        'Die Entscheidung liegt beim Arzt.</div>',
        unsafe_allow_html=True
    )

    # ── Modul 6: Signal-Freeze
    st.markdown("#### Modul 6 — Signal-Freeze Detektor")
    n_freeze = int(df["freeze_flag"].sum()) if "freeze_flag" in df.columns else 0
    if n_freeze == 0:
        st.markdown('<div class="info-card">✅ Kein kritisches Signal-Einfrieren detektiert.</div>',
                    unsafe_allow_html=True)
    else:
        freeze_pct = 100 * n_freeze / len(df)
        card_cls = "danger-card" if freeze_pct > 2 else "warn-card"
        st.markdown(
            f'<div class="{card_cls}">⚠ <strong>{n_freeze} Messpunkte ({freeze_pct:.2f}%)</strong> '
            f'mit lokalem σ &lt; 0.15 mg/dL — Signal möglicherweise "totgefiltert".<br>'
            f'<small>Interpretation: Der Sensor liefert in diesen Phasen nur noch den '
            f'Modell-Mittelwert, keine echten Messwerte.</small></div>',
            unsafe_allow_html=True
        )
        # Plot: lokale Sigma über Zeit
        if "local_sigma" in df.columns:
            fig_f, ax_f = plt.subplots(figsize=(14, 3))
            fig_f.patch.set_facecolor(COLORS["surface"])
            ax_f.set_facecolor("#FAFBFD")
            ax_f.plot(df["datetime"], df["local_sigma"],
                      color=COLORS["primary"], linewidth=0.8, alpha=0.8, label="Lokales σ")
            ax_f.axhline(0.15, color=COLORS["danger"], linewidth=1.5,
                         linestyle="--", label="Schwelle 0.15 mg/dL")
            freeze_pts = df[df["freeze_flag"]]
            if len(freeze_pts):
                ax_f.scatter(freeze_pts["datetime"], freeze_pts["local_sigma"],
                             c=COLORS["danger"], s=12, zorder=5, alpha=0.8)
            ax_f.set_ylabel("Lokales σ (mg/dL)", color=COLORS["muted"], fontsize=9)
            ax_f.set_title("Signal-Freeze: Lokale Standardabweichung über Zeit",
    ...
    # ── Modul 7: RoC-Sättigung
    st.markdown("#### Modul 7 — RoC-Sättigungsprüfung (Dynamic Lag Alert)")
    n_sat = int(df["roc_saturation_flag"].sum()) if "roc_saturation_flag" in df.columns else 0
    if n_sat == 0:
        st.markdown('<div class="info-card">✅ Keine RoC-Sättigung detektiert.</div>',
                    unsafe_allow_html=True)
    else:
        sat_pct = 100 * n_sat / len(df)
        st.markdown(
            f'<div class="warn-card">⚠ <strong>{n_sat} Punkte ({sat_pct:.2f}%)</strong> '
            f'mit konstanter Änderungsrate (±0.05 mg/dL Toleranz über ≥3 Punkte).<br>'
            f'<small>Interpretation: Der Hardware-Filter begrenzt möglicherweise '
            f'die Änderungsrate künstlich — der wahre Glukosewert bewegt sich '
            f'schneller als der angezeigte.</small></div>',
            unsafe_allow_html=True
        )

    st.markdown('<hr class="divider">', unsafe_allow_html=True)

    # ── Modul 8: Beschleunigungsanalyse
    st.markdown("#### Modul 8 — Beschleunigungsanalyse (2. Ableitung)")
    n_accel = int(df["accel_flag"].sum()) if "accel_flag" in df.columns else 0
    if n_accel == 0:
        st.markdown('<div class="info-card">✅ Keine abrupten Beschleunigungsknicke detektiert.</div>',
                    unsafe_allow_html=True)
    else:
        accel_pct = 100 * n_accel / len(df)
        st.markdown(
            f'<div class="warn-card">⚠ <strong>{n_accel} Punkte ({accel_pct:.2f}%)</strong> '
            f'mit abrupten Kurvenform-Knicken (normalisierte Beschleunigung &gt; 2.0).<br>'
            f'<small>Interpretation: Natürliche Glukoseverläufe haben weiche Übergänge. '
            f'Abrupte Knicke in der Beschleunigung bei konstanter Geschwindigkeit '
            f'sind ein Hinweis auf aggressive Filter-Artefakte.</small></div>',
            unsafe_allow_html=True
        )

        if "accel_norm" in df.columns:
            fig_a, ax_a = plt.subplots(figsize=(14, 3))
            fig_a.patch.set_facecolor(COLORS["surface"])
            ax_a.set_facecolor("#FAFBFD")
            plot_data = df["accel_norm"].clip(0, 10)
            ax_a.plot(df["datetime"], plot_data,
                      color=COLORS["primary"], linewidth=0.6, alpha=0.7)
            ax_a.axhline(2.0, color=COLORS["danger"], linewidth=1.5,
                         linestyle="--", label="Schwelle 2.0")
            accel_pts = df[df["accel_flag"]]
            if len(accel_pts):
                ax_a.scatter(accel_pts["datetime"],
                             accel_pts["accel_norm"].clip(0, 10),
                             c=COLORS["danger"], s=12, zorder=5, alpha=0.8)
            ax_a.set_ylabel("Norm. Beschleunigung", color=COLORS["muted"], fontsize=9)
            ax_a.set_title("Normalisierte 2. Ableitung des Glukosesignals",
                           color=COLORS["text"], fontsize=11, fontweight="bold")
    ...
    # ── Modul 9: Mahlzeiten-Stress-Test
    st.markdown("#### Modul 9 — Mahlzeiten-Stress-Test")
    if len(bolus_df) == 0:
        st.markdown(
            '<div class="info-card">ℹ Keine Bolus-Daten verfügbar. '
            'Der Mahlzeiten-Stress-Test erfordert eine Glooko-ZIP mit bolus_data.csv.</div>',
            unsafe_allow_html=True
        )
    elif len(meal_stress_df) == 0:
        st.markdown(
            f'<div class="info-card">✅ Alle {len(bolus_df)} analysierten Mahlzeiten '
            f'zeigen einen messbaren prandialen Anstieg — kein Hardware-Bias detektiert.</div>',
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            f'<div class="danger-card">⚠ <strong>{len(meal_stress_df)} von {len(bolus_df)} '
            f'Mahlzeiten</strong> ohne messbaren prandialen Anstieg innerhalb von 25 Minuten.<br>'
            f'<small>Mögliche Ursache: Hardware-Filter unterdrückt postprandialen Anstieg '
            f'("verschluckt" den prandialen Gradienten). Klinische Einordnung erforderlich.</small></div>',
            unsafe_allow_html=True
        )
        st.dataframe(meal_stress_df, hide_index=True, use_container_width=True)

    # ── Gesamt-Zusammenfassung
    st.markdown('<hr class="divider">', unsafe_allow_html=True)
    st.markdown("#### Gesamt-Übersicht Signal-Qualität")
    summary_data = {
        "Modul": [
            "M6 — Signal-Freeze",
            "M7 — RoC-Sättigung",
            "M8 — Beschleunigungsknick",
            "M9 — Mahlzeiten-Stress-Test",
        ],
        "Befunde": [
            f"{int(df['freeze_flag'].sum()) if 'freeze_flag' in df.columns else 0} Punkte",
            f"{int(df['roc_saturation_flag'].sum()) if 'roc_saturation_flag' in df.columns else 0} Punkte",
            f"{int(df['accel_flag'].sum()) if 'accel_flag' in df.columns else 0} Punkte",
            f"{len(meal_stress_df)} Mahlzeiten" if len(bolus_df) > 0 else "Keine Daten",
        ],
        "Bewertung": [
            "⚠ Prüfen" if (df["freeze_flag"].sum() if "freeze_flag" in df.columns else 0) > 0 else "✓ OK",
            "⚠ Prüfen" if (df["roc_saturation_flag"].sum() if "roc_saturation_flag" in df.columns else 0) > 0 else "✓ OK",
            "⚠ Prüfen" if (df["accel_flag"].sum() if "accel_flag" in df.columns else 0) > 0 else "✓ OK",
            "⚠ Prüfen" if len(meal_stress_df) > 0 else ("✓ OK" if len(bolus_df) > 0 else "— n/a"),
        ],
        "Interpretation": [
            "Lokales σ < 0.15 mg/dL über ≥5 Punkte",
            "Konstante RoC ±0.05 mg/dL über ≥3 Punkte",
    ...
    st.markdown(
        ...
        '<strong>Wichtiger Hinweis:</strong> Diese Module erkennen mögliche '
        'Hardware-induzierte Bias-Effekte. Sie ersetzen keine klinische Beurteilung. '
        'Ein positiver Befund ist ein Hinweis, kein Beweis. '
        'Die endgültige Interpretation liegt beim behandelnden Arzt.</div>',
        unsafe_allow_html=True
    )
...
with tab7:
    ...
    with st.spinner("Report wird erstellt…"):
        pdf_bytes = generate_pdf_report(
            ...
            meal_stress_df=meal_stress_df, bolus_df=bolus_df
        )
